pipeline {
    ...
    stages {
        stage('Clone Repository') {
            steps {
                git branch: 'main', url: 'https://github.com/appasabkambale/freelance-frontend.git'
            }
        }
        ...
        withCredentials([sshUserPrivateKey(credentialsId: 'ec2-ssh-key', keyFileVariable: 'KEY')]) {
            sh """
                ...
                scp -i \$KEY -o StrictHostKeyChecking=no -r \
                    src public \
                    Dockerfile package.json package-lock.json vite.config.js .env index.html\
